...
def validate_video_file(video_path):
    """Validate that the video file has a video stream."""
    import subprocess
    ...
    if file_size < 1000:  # Less than 1KB is probably not a valid video
        raise ValueError(f"Video file too small ({file_size} bytes), likely corrupted or audio-only")
    ...
    try:
        ...
        if 'video' not in result.stdout.lower():
            # No video stream found, check if it's audio-only
            audio_cmd = [
                'ffprobe', '-v', 'error',
                '-select_streams', 'a:0',
                '-show_entries', 'stream=codec_type',
            ...
            if 'audio' in audio_result.stdout.lower():
                raise ValueError(
                    f"File is audio-only (no video stream). "
                    f"Please upload a video file, not an audio file."
                )
            else:
                raise ValueError(f"No valid video or audio stream found in file")
        
        return True
    except subprocess.TimeoutExpired:
        raise ValueError("Timeout validating video file")
    except ValueError:
        raise
    except Exception as e:
        raise ValueError(f"Error validating video: {e}")
...
Technical Details: {str(e)}
    ...
    ydl_opts = {
        **_COMMON_YDL_OPTS,
        'format': 'bestvideo[ext=mp4][vcodec^=avc1]+bestaudio[ext=m4a]/bestvideo[ext=mp4]+bestaudio/best[ext=mp4]/bestvideo+bestaudio/best',
...
def process_video_to_vertical(input_video, final_output_video):
    ...
    print("   Step 0: Validating video file...")
    try:
...
def transcribe_video(video_path):
    """Transcribe video with chunked processing, resume, and cache."""
    import json
    import subprocess
    from faster_whisper import WhisperModel
    
    CHUNK_DURATION = 600  # 10 minutes per chunk
    PROGRESS_FILE = f"{video_path}.transcript_progress.json"
    CACHE_FILE = f"{video_path}.transcript.json"
    
    # Check for complete transcript cache first
    if os.path.exists(CACHE_FILE):
        try:
            with open(CACHE_FILE, 'r') as f:
                cached = json.load(f)
                if cached.get('text') is not None and cached.get('segments') is not None:
                    print(f"📋 Using cached transcript: {len(cached['segments'])} segments, {len(cached['text'])} chars")
                    return cached
        except Exception as e:
    ...
    try:
        ...
        print(f"⚠️ Could not get video duration: {e}. Processing as single chunk.")
        video_duration = 0
    ...
    if os.path.exists(PROGRESS_FILE):
        try:
            with open(PROGRESS_FILE, 'r') as f:
                ...
                completed_chunks = progress_data.get('chunks', {})
                print(f"📋 Resuming transcription: {len(completed_chunks)} chunks already done")
        except Exception as e:
            print(f"⚠️ Could not load progress file: {e}. Starting fresh.")
            completed_chunks = {}
    
    # Calculate chunks needed
    if video_duration <= 0 or video_duration <= CHUNK_DURATION:
        chunks = [(0, video_duration if video_duration > 0 else None)]
        chunk_keys = ["full"]
    else:
        chunks = []
        chunk_keys = []
        ...
        while start < video_duration:
            ...
            chunk_keys.append(f"{start:.1f}_{end:.1f}")
            start = end
    ...
    for i, ((start, end), chunk_key) in enumerate(zip(chunks, chunk_keys)):
        
        # Check if this chunk was already transcribed
        if chunk_key in completed_chunks:
            print(f"✅ Chunk {i+1}/{len(chunks)}: Already transcribed, loading from cache")
            chunk_data = completed_chunks[chunk_key]
            all_segments.extend(chunk_data.get('segments', []))
            full_text += chunk_data.get('text', '') + " "
            if not detected_language and chunk_data.get('language'):
                detected_language = chunk_data['language']
            continue
        
        # Transcribe this chunk
        end_str = f"{end:.1f}s" if end else "end"
        print(f"🎙️  Chunk {i+1}/{len(chunks)}: Transcribing [{start:.1f}s - {end_str}]...")
        
        try:
            if end is None:
                segments, info = model.transcribe(video_path, word_timestamps=True)
            else:
                chunk_file = f"{video_path}.chunk_{i}.wav"
                extract_cmd = [
            ...
            for segment in segments:
                seg_dict = {
                    'text': segment.text,
                    'start': segment.start + start if end else segment.start,
                    'end': segment.end + start if end else segment.end,
                    'words': []
                }
                ...
                if segment.words:
                    for word in segment.words:
                        seg_dict['words'].append({
                            'word': word.word,
                            'start': word.start + start if end else word.start,
                            'end': word.end + start if end else word.end,
                            'probability': word.probability
                        })
                
                chunk_segments.append(seg_dict)
                chunk_text += segment.text + " "
                print(f"   [{seg_dict['start']:.2f}s -> {seg_dict['end']:.2f}s] {segment.text}")
            
            completed_chunks[chunk_key] = {
            ...
            with open(PROGRESS_FILE, 'w') as f:
                json.dump({'chunks': completed_chunks}, f, ensure_ascii=False, indent=2)
            
            all_segments.extend(chunk_segments)
            full_text += chunk_text
            
            print(f"   ✅ Chunk {i+1}/{len(chunks)} complete ({len(chunk_segments)} segments)")
    ...
    try:
        ...
        print(f"💾 Transcript cached to: {CACHE_FILE}")
    except Exception as e:
        print(f"⚠️ Could not save transcript cache: {e}")
    
    print(f"✅ Transcription complete: {len(all_segments)} segments, {len(full_text)} chars")
    
    return transcript_result

def get_viral_clips(transcript_result, video_duration, llm_client=None):
    ...
    print(f"🤖  Using {llm_client.provider} / {llm_client.model}")

    # Estimate tokens (roughly 4 chars per token)
    full_text = transcript_result['text']
    estimated_tokens = len(full_text) // 4
    
    MAX_SAFE_TOKENS = 800000  # Leave room for prompt template + response
    
    if estimated_tokens > MAX_SAFE_TOKENS:
        print(f"⚠️  Transcript too long ({estimated_tokens} estimated tokens). Compressing...")
        
        # Strategy: Use a compressed format with just key timestamps
        # Take every Nth segment to fit within limits
        segments = transcript_result['segments']
        target_segments = int(len(segments) * (MAX_SAFE_TOKENS / estimated_tokens))
        target_segments = max(target_segments, 100)  # Keep at least 100 segments
        
        step = max(1, len(segments) // target_segments)
        compressed_segments = segments[::step]
        
        # Build compressed text with timestamps
        compressed_text = ""
        words = []
        for seg in compressed_segments:
            compressed_text += f"[{seg['start']:.1f}s] {seg['text']} "
        ...
        print(f"📊 Compressed: {len(segments)} → {len(compressed_segments)} segments ({len(compressed_text)//4} estimated tokens)")
        
        prompt = GEMINI_PROMPT_TEMPLATE.format(
            video_duration=video_duration,
            transcript_text=json.dumps(compressed_text),
            words_json=json.dumps(words)
        )
    else:
        # Normal path - use full transcript
        words = []
        ...
        prompt = GEMINI_PROMPT_TEMPLATE.format(
            video_duration=video_duration,
            transcript_text=json.dumps(full_text),
            words_json=json.dumps(words)
        )
