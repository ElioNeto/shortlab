...
def analyze_video_for_titles(api_key, video_path, transcript=None, provider="gemini", model=None):
    """
    Transcribes a video and uses LLM to suggest viral YouTube titles.
    ...
    print("📤 [Thumbnail] Uploading video...")
    llm = LLMClient(provider=provider, api_key=api_key, model=model)

    file_upload = llm.upload_video(video_path)
...
{{
    ...
    print("🤖 [Thumbnail] Asking LLM for title suggestions...")
    response_text = llm.generate_text_with_video(
        video_file_obj=file_upload,
        prompt=prompt,
...
def refine_titles(api_key, context, user_message, conversation_history=None, provider="gemini", model=None):
    """
...
def generate_thumbnail(api_key, title, session_id, face_image_path=None, bg_image_path=None, extra_prompt="", count=3, video_context="", provider="gemini", model=None):
    """
    ...
    llm = LLMClient(provider=provider, api_key=api_key, model=model or "gemini-3.1-flash-image-preview")

    if not llm.supports_image_generation:
        raise NotImplementedError(
            "Thumbnail generation requires Gemini with image generation model. "
            "Set model to gemini-3.1-flash-image-preview in Settings."
        )
...
- NO clutter, NO small text, NO watermarks"""
    ...
    for i in range(count):
        ...
        try:
            response = llm.generate_text_with_images(
                image_parts=prompt_parts,
            ...
            for part in response.parts:
                if part.text is not None:
                    print(f"📝 [Thumbnail] LLM text: {part.text}")
...
def generate_youtube_description(api_key, title, transcript_segments, language, video_duration, provider="gemini", model=None):
    """
    Uses LLM to generate a YouTube description with chapter markers from transcript segments.
...
OUTPUT: Return ONLY the description text (no JSON wrapper, no markdown code blocks). The description should be ready to paste directly into YouTube."""
    ...
    description = llm.generate_text(prompt=prompt)
