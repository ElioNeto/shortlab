...
def research_saas_online(url: str, gemini_key: str, provider: str = "gemini", model: str = None) -> dict:
    """
    Use LLM with Google Search grounding to deeply research a SaaS product
    across the internet: reviews, Reddit threads, Twitter, competitor comparisons,
    pricing complaints, user testimonials, etc.
    Note: Search grounding requires Gemini provider.
    """
    ...
    if not llm.supports_search_grounding:
        print("[SaaSShorts] ⚠️ Search grounding requires Gemini. Falling back to text-only analysis.")
        return _research_saas_fallback(url, gemini_key, provider, model)
...
Be thorough. Use REAL data from your search results, not made-up information."""
    ...
    response_text, sources = llm.generate_text_with_search(
        prompt=prompt,
    ...
    if not response_text:
        print("[SaaSShorts] ⚠️ LLM returned empty response for web research")
...
def _research_saas_fallback(url: str, gemini_key: str, provider: str, model: str) -> dict:
    """Fallback research without Google Search grounding."""
    from llm import LLMClient, parse_json_response

    llm = LLMClient(provider=provider, api_key=gemini_key, model=model)
    domain = url.replace("https://", "").replace("http://", "").split("/")[0]

    prompt = f"""Research this SaaS product based on your knowledge. Provide what you know about the product, its features, pricing, competitors, and user sentiment.

Product URL: {url}
Domain: {domain}

Return a JSON research report:
{{
    "product_name": "...",
    "website_url": "{url}",
    "what_it_does": "Description of the product",
    "target_market": "Who this product is for",
    "pricing_info": "Known pricing details",
    "user_sentiment": "overall positive/mixed/negative",
    "real_reviews": [],
    "common_complaints": [],
    "common_praise": [],
    ...
    print(f"[SaaSShorts] ✅ Web research complete (fallback mode)")
    return research
...
def analyze_saas(scraped_data: dict, gemini_key: str, web_research: dict = None, provider: str = "gemini", model: str = None) -> dict:
    """
...
Include 5-8 pain points, 4-6 emotional hooks, and 4+ viral angles."""
    ...
    if not response_text:
        raise Exception("LLM returned empty response for SaaS analysis")
...
- Example male: "a 29 year old european man, short dark hair, light stubble, wearing a navy t-shirt, smart casual look" """
    ...
    if not response_text:
        raise Exception("LLM returned empty response for script generation")

    try:
        scripts = parse_json_response(response_text)
    except json.JSONDecodeError as e:
