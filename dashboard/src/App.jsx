...
const SESSION_KEY = 'shortlab_session';
  ...
  const handleProcess = async (data) => {
    ...
    try {
      ...
      const res = await fetch(getApiUrl('/api/process'), {
        method: 'POST',
        headers: data.type === 'url' ? headers : { 'X-Gemini-Key': apiKey, 'X-LLM-Provider': llmProvider, 'X-LLM-Model': llmModel },
        body
      });
    ...
    <div className="w-20 lg:w-64 bg-surface border-r border-white/5 flex flex-col h-full shrink-0 transition-all duration-300">
      <div className="p-6 flex items-center gap-3">
        <div className="w-8 h-8 bg-white/5 rounded-lg flex items-center justify-center shrink-0 overflow-hidden border border-white/5">
          <img src="/logo-shortlab.png" alt="Logo" className="w-full h-full object-cover" />
        </div>
        <span className="font-bold text-lg text-white hidden lg:block tracking-tight">ShortLab</span>
      </div>
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden relative">
        ...
        {!apiKey && activeTab !== 'settings' && (
          <div className="mx-6 mt-3 p-3 bg-amber-500/10 border border-amber-500/30 rounded-xl flex items-center justify-between gap-4 shrink-0 animate-[fadeIn_0.3s_ease-out]">
            <div className="flex items-center gap-3 text-sm text-amber-200">
              ...
              <div>
                <span className="font-semibold">LLM API key missing.</span>{' '}
                <span className="text-amber-200/80">
                  Set your LLM API key to use ShortLab.
                </span>
              </div>
            </div>
            <button
        ...
        <div className="flex-1 overflow-hidden relative">
          ...
          {activeTab === 'saasshorts' && (
            <SaaShortsTab geminiApiKey={apiKey} elevenLabsKey={elevenLabsKey} falKey={falKey} uploadPostKey={uploadPostKey} uploadUserId={uploadUserId} llmProvider={llmProvider} llmModel={llmModel} />
          )}
          ...
          {activeTab === 'thumbnails' && (
            <ThumbnailStudio geminiApiKey={apiKey} uploadPostKey={uploadPostKey} uploadUserId={uploadUserId} llmProvider={llmProvider} llmModel={llmModel} />
          )}
      ...
      {showKeyModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setShowKeyModal(false)}>
          <div className="bg-[#18181b] border border-white/10 rounded-2xl p-6 max-w-md w-full mx-4 space-y-4 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="text-sm text-zinc-400">
              ShortLab needs an <strong className="text-zinc-200">LLM API key</strong> (Gemini or OpenRouter) to power the AI analysis.
            </p>
            ...
            <div className={`rounded-lg p-4 space-y-2 border ${!apiKey ? 'bg-blue-500/5 border-blue-500/30' : 'bg-white/5 border-white/10 opacity-70'}`}>
              <p className="text-xs font-semibold text-zinc-200 flex items-center gap-2">
                ...
                LLM API Key {apiKey && <span className="text-green-400">— set</span>}
              </p>
              {!apiKey && (
                <>
                  <p className="text-xs text-zinc-400">
                    Use Google Gemini (free tier) or OpenRouter (free models available).
                  </p>
                  <input
                    type="text"
            ...
            {/* Upload-Post block (optional) */}
            <div className="rounded-lg p-4 space-y-2 border bg-white/5 border-white/10 opacity-70">
              <p className="text-xs font-semibold text-zinc-200 flex items-center gap-2">
                {uploadPostKey ? <Check size={12} className="text-green-400" /> : <span className="text-zinc-500">○</span>}
                Upload-Post API Key <span className="text-zinc-500 text-xs">(optional — for social publishing)</span>
              </p>
              <p className="text-xs text-zinc-400">
                Enables direct publishing to TikTok, Instagram Reels, and YouTube Shorts. Free tier: 10 uploads/month.
              </p>
              {uploadPostKey && <span className="text-xs text-green-400">— configured</span>}
            </div>
